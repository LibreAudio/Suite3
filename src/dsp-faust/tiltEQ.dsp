...
//----------------------------------------------------------------- UI
mode   = uiTop(nentry("[0] Mode [style:menu{'Linear':0;'Shelf + Resonance':1;'Dual Shelf':2}][symbol:mode]", 0, 0, 2, 1));

// shared by every mode
tilt   = uiBottomCenter(hslider("[1] Tilt [unit:dB][style:knob][symbol:tilt][easy][label:Tilt][accentcolor:01]", 0, -12, 12, 0.1)) : si.smoo;
freq   = uiBottomCenter(hslider("[2] Frequency [unit:Hz][style:knob][scale:log][symbol:freq][label:Center][accentcolor:03]", 630, 20, 20000, 1)) : si.smoo;

// per-mode
res    = uiBottomCenter(hslider("[3] Resonance [style:knob][symbol:res][label:Q][accentcolor:06]", 0.707, 0.4, 8, 0.01)) : si.smoo;         // Shelf + Resonance
spread = uiBottomCenter(hslider("[4] Spread [style:knob][symbol:spread][label:Spread][accentcolor:04]", 2, 1, 20, 0.01)) : si.smoo;               // Dual Shelf
dsres  = uiBottomCenter(hslider("[5] DS Resonance [style:knob][unit:dB][symbol:ds_res][label:Q][accentcolor:06]", 0, 0, 12, 0.1)) : si.smoo; // Dual Shelf
