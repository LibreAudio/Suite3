...
#if 0
    // No antialiasing. Repositions the rectangles just a bit.
    //pos = step(0., vec2(width, height)/2. - abs(uv - pos));
#else
    // Antialiased equivalent. Takes a little size off the retangles,
    // so it's fudged (badly) to account for that.
